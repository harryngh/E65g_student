...
/*
    Ignore the external name of parameters
 */
/*:
...
/*
  (Int, Int) -> T: transform has the function type which takes two integer parameters and return an integer.
 */
/*:
...
/*
 T is the placeholder for a type
 */
...
/*
  offsets is an array contains the offsets to the location of neighbors of a cell
 */
/*:
 ## Problem 6:
...
/*
  We use the word "of" when we call the function neighbors because it is the external name
 */
/*:
...
/*
  We use the word "cell" in the function implementation because it is the local name
 */
...
extension Grid {
    ...
    func neighbors(of cell: Cell) -> [Position] {
        return Grid.offsets.map {
            ...
            return Position(
              ((cell.position.row + $0.row) % self.rows + self.rows) % self.rows,
              ((cell.position.col + $0.col) % self.cols + self.cols) % self.cols)
        }
    }
}
/*:
...
/*
  "combine" is a function that takes three argument, it do some computation of the first arugment and the value specified by the second and third arguments.
 */
/*:
...
/*
  reduce2 is a specific function thus it is not genericized by parameter T
 */
...
extension Grid {
    var numLiving: Int {
        return reduce2(self.rows, self.cols) { total, row, col in
          ...
          return total + (self.cells[row][col].state.isAlive ? 1: 0);
        }
    }
}
/*:
...
extension Grid {
    subscript (row: Int, col: Int) -> Cell? {
        get {
          ...
          guard row<self.rows && row>=0 else {return nil}
          guard col<self.cols && col>=0 else {return nil}
          return self.cells[row][col];
          
        }
        set {
            // ** Your Problem 14 `set` code goes here! replace the following line **
          guard row<self.rows && row>=0 else {return}
          guard col<self.cols && col>=0 else {return}
...
/*
    Because subscript return an optional Cell
 */
/*:
...
/*
    When $1.row or $1.col does not fall between of appropriate row and column, but under this circumstance -> NEVER.
 */
/*:
 ## Problem 16:
...
/*
  reduce function returns the number of living neighbors of a cell.
 */
...
/*
  $1 is one of positions of its neighbors.
 */
...
extension Grid {
    func next() -> Grid {
        ...
        map2(self.rows, self.cols) { (row, col)  in
          switch self.nextState(of: self[row,col]!){
          case .alive:
            nextGrid[row,col]?.state = .alive
          case .empty:
            fallthrough
          default:
            nextGrid[row,col]?.state = .empty
          }
        }
        return nextGrid
    }
...
/*
  nextgrid is the state of the grid in the next cycle.
 */
/*:
 ## Problem 22:
